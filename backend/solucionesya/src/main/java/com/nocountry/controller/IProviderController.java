...
public interface IProviderController {
    ...
    ResponseEntity<ProviderResponse> addFileToProvider(@NotNull @PathVariable("id-provider") String idProvider,
                                                       @RequestParam(value = "image", required = false) MultipartFile image) throws ProviderException, ImageException;

    @DeleteMapping(path = "/remove-image-to-provider/{id-provider}/image/{id-image}", produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseEntity<ProviderResponse> removeFileToProvider(@NotNull @PathVariable("id-provider") String idProvider,
                                                          @NotNull @PathVariable("id-image") String idImage) throws ImageException, ProviderException;
}